...
class RoleController extends Controller {
  ...
  async update() {
    const { ctx } = this;
    const result = await ctx.service.role.updateOne(ctx.params.id, ctx.request.body);
    if (!result) {
  ...
  async destroy() {
    const { ctx } = this;
    const result = await ctx.service.role.deleteOne(ctx.params.id);
    ctx.body = result;
  }

  async enum() { // 获取枚举数据
    const { ctx } = this;
    const result = await ctx.service.role.enum();
    ctx.body = result;
  }
}
