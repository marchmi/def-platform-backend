...
class InterfaceController extends Controller {
  async index() {
    ...
    const result = await ctx.service.interface.find();
  ...
  async create() {
    const { ctx } = this;
    const result = await ctx.service.interface.create(ctx.request.body);
    ctx.body = result;
  }

  async update() {
    const { ctx } = this;
    const result = await ctx.service.interface.updateOne(ctx.params.id, ctx.request.body);
    if (!result) {
  ...
  async destroy() {
    const { ctx } = this;
    const result = await ctx.service.interface.deleteOne(ctx.params.id);
    ctx.body = result;
  }

  async enum() { // 获取枚举数据
    const { ctx } = this;
    const { sysCode } = ctx.params;
    if (!sysCode) {
      ctx.body = {
        code: 204,
        message: '缺少sysCode参数',
    ...
    const result = await ctx.service.interface.enum(sysCode);
    ctx.body = result;
  }
}
