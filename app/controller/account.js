...
class AccountController extends Controller {
  async index() {
    ...
    const result = await ctx.service.account.find();
  ...
  async create() {
    const { ctx } = this;
    const result = await ctx.service.account.create(ctx.request.body);
    ctx.body = result;
  }

  async update() {
    const { ctx } = this;
    const result = await ctx.service.account.updateOne(ctx.params.id, ctx.request.body);
    if (!result) {
  ...
  async destroy() {
    const { ctx } = this;
    const result = await ctx.service.account.deleteOne(ctx.params.id);
    ctx.body = result;
  }
}
