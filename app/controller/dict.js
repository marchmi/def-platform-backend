...
class DictController extends Controller {
  async index() {
    ...
    const result = await ctx.service.dict.find();
  ...
  async create() {
    const { ctx } = this;
    const result = await ctx.service.dict.create(ctx.request.body);
    ctx.body = result;
  }

  async update() {
    const { ctx } = this;
    const result = await ctx.service.dict.updateOne(ctx.params.id, ctx.request.body);
    if (!result) {
  ...
  async destroy() {
    const { ctx } = this;
    const result = await ctx.service.dict.deleteOne(ctx.params.id);
    ctx.body = result;
  }
}
