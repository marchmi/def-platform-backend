...
class ComponentsController extends Controller {
  async index() {
    ...
    const result = await ctx.service.components.find();
  ...
  async create() {
    const { ctx } = this;
    const result = await ctx.service.components.create(ctx.request.body);
    ctx.body = result;
  }

  async update() {
    const { ctx } = this;
    const result = await ctx.service.components.updateOne(ctx.params.id, ctx.request.body);
    if (!result) {
  ...
  async destroy() {
    const { ctx } = this;
    const result = await ctx.service.components.deleteOne(ctx.params.id);
    ctx.body = result;
  }

  async enum() { // 获取枚举数据
    const { ctx } = this;
    const result = await ctx.service.components.enum();
    ctx.body = result;
  }
}
