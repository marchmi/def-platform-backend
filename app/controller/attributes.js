...
class AttributesController extends Controller {
  async index() {
    ...
    const result = await ctx.service.attributes.find();
  ...
  async create() {
    const { ctx } = this;
    const result = await ctx.service.attributes.create(ctx.request.body);
    ctx.body = result;
  }

  async update() {
    const { ctx } = this;
    const result = await ctx.service.attributes.updateOne(ctx.params.id, ctx.request.body);
    if (!result) {
  ...
  async destroy() {
    const { ctx } = this;
    const result = await ctx.service.attributes.deleteOne(ctx.params.id);
    ctx.body = result;
  }

  async enum() { // 获取枚举数据
    const { ctx } = this;
    const result = await ctx.service.attributes.enum();
    ctx.body = result;
  }
}
